...
export default function AnswerCard({ result, papers }: Props) {
  ...
  const { answer, claims, retrieval_quality, retries, latency_ms, query_type, hops, diagnostic } =
    result;
  ...
  return (
    ...
    >
      ...
      {diagnostic?.llm_error && (
        ...
        >
          LLM error: {diagnostic.llm_error}
        </div>
      )}
      ...
      >
          ...
          <div className="quality-bar-track" style={{ flex: 1 }}>
            <div
              ...
              style={{
                width: `${qScore}%`,
                background:
                  qScore >= 70 ? '#10b981' : qScore >= 45 ? '#f59e0b' : '#ef4444',
              }}
            />
          </div>
          <span
        ...
        <div style={{ display: 'flex', gap: 8, fontSize: 11, color: 'var(--text-muted)' }}>
          ...
          {retries > 0 && (
            <span>
              {retries} retri{retries > 1 ? 'es' : 'y'}
            </span>
          )}
          {diagnostic?.k_final != null && <span>{diagnostic.k_final} chunks</span>}
        </div>
      ...
      {/* Diagnostic */}
      {showDiag && <DiagPanel result={result} />}
      ...
      <div style={{ flex: 1 }}>
        <p
          style={{
            ...
            lineHeight: 1.6,
            marginBottom: claim.citations.length ? 6 : 0,
          }}
        >
          {claim.claim}
        </p>
  ...
  return (
    ...
    >
      ...
      >
        ...
        <Stat
          label="Citations aug"
          value={String(diagnostic.citation_augmented_count ?? 0)}
        />
        {diagnostic.llm_error && (
          <Stat label="LLM error" value="See above" color="#ef4444" />
        )}
      </div>

      {diagnostic.verification?.stage_scores &&
        Object.keys(diagnostic.verification.stage_scores).length > 0 && (
          <>
...
function Stat({ label, value, color }: { label: string; value: string; color?: string }) {
  return (
    <div>
      ...
      <p style={{ fontSize: 13, fontWeight: 600, color: color || 'var(--text-primary)' }}>
        {value}
      </p>
    </div>
  );
